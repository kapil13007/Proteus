...
## Using your own table pair

### Schema MD format

Both source and target schema files follow this format exactly:

```markdown
# Table: raw.your_table_name

| column | type | nullable | description |
|---|---|---|---|
| id | VARCHAR | NO | Primary key |
| name | VARCHAR | YES | Full name |
| created_at | TIMESTAMP | YES | Record creation time |
```

The table must already exist in your Postgres instance.

### STTM sheet columns

Your CSV or Excel file needs these headers (case and spacing tolerant):
...
Only rows where `mapping_status` is `Validated` (or blank) are executed. `In Progress`, `Failed`, and other statuses are excluded automatically.

### Repo structure MD format
...
## Notes
- Any company-specific rules the agent should follow
```

### UDF definitions MD format

```markdown
# Dataform UDF Definitions

## functionName(args)
- File: includes/helpers.js
- Purpose: what it does
- When to use: when to apply it
- Example: ${helpers.functionName("column")}
```
...
Tests the full pipeline against real Postgres with a stubbed LLM. Attempt 1 contains a deliberate bad column name to prove the dry-run self-correction loop fires:

```bash
cd backend
# Edit DATABASE_URL at the top of test_e2e.py if your port differs
python test_e2e.py
```

Expected output ends with `ALL E2E CHECKS PASSED`.
...
| `GITHUB_TOKEN` | No | Fine-grained PAT with Contents read/write |
| `GITHUB_REPO` | No | e.g. `username/dataform-models` |
| `GITHUB_BRANCH` | No | Default: `main` |
| `LANGSMITH_TRACING` | No | `true` to enable tracing |
| `LANGSMITH_API_KEY` | No | LangSmith API key |
| `LANGSMITH_PROJECT` | No | Default: `mapflow` |
| `MAX_DRY_RUN_ATTEMPTS` | No | Default: `3` |
| `SOURCE_SCHEMA` | No | Default: `raw` |
| `TARGET_SCHEMA` | No | Default: `analytics` |

---

## Roadmap

**v1 — current (local Postgres)**
- [x] LangGraph ReAct agent with self-correction loop
- [x] Deterministic validator (existence, types, nullability, duplicates, status filtering)
- [x] LLM generation from real schema + STTM + repo conventions + UDF context
- [x] Postgres EXPLAIN dry-run with bounded retry
- [x] Human review gate with editable `.sqlx`
- [x] GitHub push (`.sqlx` at correct repo path)
- [x] SQL execution against local Postgres
- [x] Post-load audit (row counts, null checks)
- [x] LangSmith tracing
- [x] CSV and Excel STTM support

**v2 — planned**
- [ ] User auth (JWT)
- [ ] Per-user database connections (bring your own Postgres URI)
- [ ] BigQuery + Dataform swap (replace local Postgres tools)
- [ ] MCP server architecture (tools as standalone services)
- [ ] Planner agent (multi-table join inference)
- [ ] Audit agent (LLM-as-judge semantic spot checks)
